<template>
  <div class="page-container">
    ...
    <div class="search-form">
      <a-form layout="inline" :model="queryParams">
        <a-form-item label="菜单名称">
          <a-input v-model:value="queryParams.name" placeholder="请输入菜单名称" allow-clear />
        </a-form-item>
    ...
    <div class="table-toolbar">
      <a-space>
        <a-button type="primary" v-permission="'system:menu:add'" @click="handleAdd()">
          <PlusOutlined /> 新增
        </a-button>
      ...
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'action'">
          <a-space>
            ...
            <a-button type="link" size="small" v-permission="'system:menu:edit'" @click="handleEdit(record)">
              编辑
            </a-button>
            <a-popconfirm title="确定要删除吗？" @confirm="handleDelete(record.id)">
              <a-button type="link" size="small" danger v-permission="'system:menu:delete'">
                删除
              </a-button>
            </a-popconfirm>
          </a-space>
...
const menuTypeText: Record<number, string> = {
  0: '目录',
  1: '菜单',
  2: '按钮',
};

const menuTypeColor: Record<number, string> = {
  0: 'blue',
  1: 'green',
  2: 'orange',
};
...
async function fetchData() {
  loading.value = true;
  try {
    const list = await getMenuList(queryParams);
